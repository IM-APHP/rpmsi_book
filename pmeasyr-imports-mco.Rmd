...
pmeasyr::formats %>% dplyr::filter(table == "tra") %>% 
  dplyr::group_by(champ,an) %>% dplyr::summarise(nb = dplyr::n() ) 

pmeasyr::formats %>% dplyr::group_by(champ,table) %>% 
  dplyr::summarise(nb_annee = length(unique(an)))
...
Le fichier AMURM est maintenu par le DIM du siège et comprend de nombreuses données utiles sur les UM. Dans sa forme actuelle il comprend également les UA qui sont le niveau en dessous de l'UMA. Dans la mesure où il peut y avoir plusieurs UA par UMA, il peut contenir plusieurs lignes pour une seule UMA. Dans ce fichier les UMA entité juridique sont dénommées ```uma_ej``` alors que dans le nom de cette variable importée avec ```pmeasyr``` est ```cdurm``` .
...
data21 <- data21 %>%
  dplyr::left_join( .,
                   amurm21 %>% dplyr::select( gh, cdurm, typaut, mode_hospit, nohop, 
